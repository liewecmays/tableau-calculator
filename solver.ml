...
let rec create_nodes_from_formula (fml, world) relations =
...
- relations: 探索の中で出てきた到達可能性関係のリスト
...
let rec search_tableau node_list relations boxes branch =
	match node_list with
	| [] ->
		let res = find_valuation branch in
			if is_debug_mode then
				match res with
				| Some v -> print_endline_in_tree ("→ valuation found: " ^ string_of_valuation v Debug) !level
	...
	| (fml, world) :: rest ->
		...
		match fml with
		| FVar x -> search_tableau rest relations boxes ((x, world, true) :: branch) (* 原子論理式の場合はbranchに追加 *)
		| FIf (fml1, fml2) ->
			if is_debug_mode then (print_string_in_tree "- " !level; level := !level + 2; after_fork := true) else (); (* 分岐する場合はlevelを深くする *)
			(match search_tableau ((FNot fml1, world) :: rest) relations boxes branch with (* 前半部分の探索 *)
			...
			| None -> 
				...
				let res = search_tableau ((fml2, world) :: rest) relations boxes branch in (* 後半部分の探索 *)
					if is_debug_mode then level := !level - 2 else (); (* levelを戻す *)
					res)
		| FAnd (fml1, fml2) -> search_tableau ((fml1, world) :: (fml2, world) :: rest) relations boxes branch
		| FOr (fml1, fml2) ->
			if is_debug_mode then (print_string_in_tree "- " !level; level := !level + 2; after_fork := true) else ();
			(match search_tableau ((fml1, world) :: rest) relations boxes branch with
			| Some v -> Some v
			| None -> 
				if is_debug_mode then (level := !level - 2; print_string_in_tree "- " !level; level := !level + 2; after_fork := true) else ();
				let res = search_tableau ((fml2, world) :: rest) relations boxes branch in
					if is_debug_mode then level := !level - 2 else ();
					res)
		| FBox fml' ->
			let new_nodes = create_nodes_from_formula (fml', world) relations in (* この論理式と世界に対して到達可能性関係をもとにノードを増やす *)
				search_tableau (new_nodes @ rest) relations ((fml', world) :: boxes) branch
		| FDia fml' ->
			...
			let res = search_tableau (((fml', new_world) :: new_nodes) @ rest) (new_relation :: relations) boxes branch in
				world_count := !world_count - 1;
				res
		| FNot fml' -> (* 否定記号から始まる場合 *)
			match fml' with
			| FVar x -> search_tableau rest relations boxes ((x, world, false) :: branch)
			| FIf (fml1, fml2) -> search_tableau ((fml1, world) :: (FNot fml2, world) :: rest) relations boxes branch
			| FAnd (fml1, fml2) ->
				if is_debug_mode then (print_string_in_tree "- " !level; level := !level + 2; after_fork := true) else ();
				(match search_tableau ((FNot fml1, world) :: rest) relations boxes branch with
				| Some v -> Some v
				| None -> 
					if is_debug_mode then (level := !level - 2; print_string_in_tree "- " !level; level := !level + 2; after_fork := true) else ();
					let res = search_tableau ((FNot fml2, world) :: rest) relations boxes branch in
						if is_debug_mode then level := !level - 2 else ();
						res)
			| FOr (fml1, fml2) -> search_tableau ((FNot fml1, world) :: (FNot fml2, world) :: rest) relations boxes branch
			| FNot fml'' -> search_tableau ((fml'', world) :: rest) relations boxes branch
			| FBox fml'' -> search_tableau ((FDia (FNot fml''), world) :: rest) relations boxes branch
			| FDia fml'' -> search_tableau ((FBox (FNot fml''), world) :: rest) relations boxes branch
